...
- **Обновление amnezia-box не заканчивается «Failed to fetch».** Запрос на
  ...
  этого же прокси (VPN с телефона), перезапуск рвёт соединение вместе с
  запросом: браузер показывал голое «TypeError: Failed to fetch», карточка
  застывала на старой версии, хотя сервер доводил обновление до конца.
  Теперь при обрыве страница ждёт возвращения связи (до полутора минут) и
  дочитывает итог из `/api/updates/progress`: успех, либо текст ошибки. Если
  связь так и не вернулась, сообщение объясняет, что случилось и что делать.
  Ошибка отката тоже стала понятной: какая версия не стартовала, что
  прежняя восстановлена, и куда смотреть (`journalctl -u amnezia-box`).
