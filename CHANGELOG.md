...
- **Главная: разбивка рядом с графиком, CPU и память в подвале (#101).** Справа
  от графика теперь кольцо с переключателем «Клиенты / Цепочки» — считает за тот
  же период, что и график: 1 ч и 24 ч по истории, которая приходит в том же
  ответе, что и сам график, так что ни одного нового запроса; на 60 с это
  открытые прямо сейчас соединения, каждое со своего начала (как «сейчас» в
  «Разбивке») — так и подписано, чтобы не путать с окном графика. Список держит
  постоянную высоту, поэтому переключение ничего не двигает. CPU и память
  переехали в подвал к тоталам и диску; у CPU остался спарклайн 40×12 — всплеск
  и есть то, ради чего на него смотрят. Наведение на график показывает время и
  обе скорости в этой точке: плашка встаёт в конец строки легенды, на место
  подписи периода, а не бегает за курсором, закрывая разбираемую линию.
...
- **Правило больше не описывает себя как «весь трафик» (#104).** Сводка строки в
  списке маршрутов и карточка в комбинаторе знали не про все условия: правило
  только с диапазоном портов, `auth_user` (такие пишет управление
  пользователями), `process_path`, `client`, `clash_mode`, `ip_version`,
  `user`/`user_id` или приватным адресом источника выглядело как «весь трафик» —
  а с действием «блокировать» и вовсе как рубильник на весь бокс. В комбинаторе
  та же карточка писала «условия не заданы», хотя предупреждения о пустой
  карточке справедливо не ставила. Недостающие условия добавлены в обе сводки.

- **Тег набора правил, созданного прямо из формы правила, сохраняется.** Форма
  условий читает переданные условия один раз при открытии и дальше владеет ими
  сама, поэтому дописанный ей снаружи тег не отмечался в списке и стирался
  ближайшей синхронизацией — правило уходило на сохранение без только что
  созданного набора.
...
- **Счётчик соединений рядом с адресом клиента (#107).** В свёрнутой строке
  ...
  висело в пустоте. Теперь оно идёт сразу за адресом — тем же порядком, что и в
  мобильной карточке. Обрезается только имя: адрес и счётчик остаются на месте, а
  то у длинного имени строка заканчивалась многоточием и числом, которое читалось
  как название.

- **В «Разбивке» и «Клиентах» только устройства этого бокса (#102).** sing-box
  изредка отдаёт в списке соединений источник с публичным адресом — в отчёте это
  была пара фронтендов Google на полкилобайта. И сэмплер истории, и обнаружение
  клиентов писали `sourceIP` как есть, так что такой адрес становился строкой
  «клиента», которого некому назвать. Обе страницы (обе — только для режима
  роутера) теперь читают через проверку на локальность: LAN, заданная подсеть
  AWG, 100.64.0.0/10, loopback и link-local остаются, остальное не показывается —
  включая уже накопленное. Живой режим «Разбивки» читает поток Clash напрямую,
  поэтому фильтруется и он: иначе страница отвечала бы на свой же переключатель
  периода двумя способами. IPv6-адрес клиента маршрутизируем по своей природе,
  по нему устройство от чужого не отличить — такие источники остаются все.
  Соединения без источника (собственные вызовы бокса, в панели «unknown») тоже
  остаются везде. Оговорка: заданную вручную непубличную подсеть AWG знает только
  бэкенд, поэтому в живом режиме «Разбивки» такой пир не виден — в периодах 1 ч и
  24 ч виден. В режиме роутера отфильтрованное больше и не записывается: ни в
  историю трафика, ни в список клиентов. В режиме VPS пишется и показывается всё
  как раньше — там публичный адрес подключившегося и есть источник, и из этих же
  строк складывается график на главной, который не фильтруется никогда.
