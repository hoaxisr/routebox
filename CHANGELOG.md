...
- **Замер скорости для endpoints (#92).** Кнопка-спидометр появилась в строке
  ...
  тем же инструментом. Точка API — `POST /api/endpoints/{tag}/speedtest`.
...
- **Ядерный AWG-сервер снова доступен там, где хост его тянет (#93).** Выбор
  ...
  Установка переживает оборванный запрос: apt и сборка DKMS больше не убиваются
  на полпути, если вкладку закрыли или POST отвалился по таймауту (потолок —
  30 минут). Пока идёт установка, панель показывает фазу («Ставим модуль
  ядра…») и состояние модуля вместо неподвижной кнопки, а шаг «Готовность» в
  мастере больше не рисует зелёную галочку над `not-installed`.
...
- **Замер скорости переехал из Outbounds в Proxies (#90).** Меряют то, что
  ...
  участника; Reject/Block кнопки не имеют — мерить нечего.
...
- **Тест скорости больше не падает на хосте с AWG-сервером (#91).** Замер идёт
  ...
  обслуживать. Сами endpoints остаются — для AWG/WireGuard endpoint и есть
  outbound.
...
- **Правки по мелочам в интерфейсе (#90).** Коды возврата DNS («Fall back on»)
  ...
  местом; у кнопки «+ Add a server» убрано подчёркивание при наведении. В
  мониторе трафика приём теперь слева от отдачи — его смотрят первым. В списке
  соединений имя клиента, его адрес и число соединений разложены по колонкам:
  длинное имя больше не сдвигает соседние строки. Результат замера скорости
  подписан словами вместо `RPM 395 / 584`.
