...
- **Обновление amnezia-box не заканчивается «Failed to fetch».** Запрос на
  обновление ждёт перезапуска прокси, и если панель открыта через туннель
  этого же прокси (VPN с телефона) или за реверс-прокси с коротким таймаутом,
  ответ теряется: браузер показывал голое «TypeError: Failed to fetch»,
  карточка застывала на старой версии, хотя сервер доводил обновление до
  конца. Теперь страница дожидается итога через `/api/updates/progress`:
  ждёт возвращения связи до трёх минут, различает «запрос до сервера не
  дошёл» (ничего не изменено, можно повторить), успех и ошибку с текстом.
  Ошибки больше не гаснут через пять секунд, а последняя неудачная попытка
  видна на карточке и после перезагрузки страницы. Прогресс получил `seq`,
  чтобы итог прошлого обновления не выдавался за свежий.
- **Ошибка отката читается.** Вместо «restart with new binary failed, rolled
  back» — какая версия не стартовала, что прежняя восстановлена и работает,
  и причина; под systemd к ней прилагается хвост журнала юнита.
- **Дашборд без прокрутки на десктопе.** На экранах от 1024 px страница
  занимает ровно высоту окна: ужимается только список Top Connections
  (прокручивается внутри, минимум две строки), всё остальное сохраняет
  размер. На слишком низком окне страница честно прокручивается, а не
  накладывает график на ссылки. Красная полоса «панель по HTTP» больше не
  показывается в режиме router: панель роутера живёт в LAN и наружу не
  выставляется, а полоса лишь отбирала строку высоты.
- **Перезапуск под systemd проверяется дольше.** После `systemctl restart`
  RouteBox две секунды (как при reload) следит, что юнит `active`, а не
  500 мс: бинарник 1.15, падающий на старте позже, раньше засчитывался
  как успешно обновлённый.

## [0.37.0]

### Features

- **TrustTunnel как клиентский outbound.** Форк amnezia-box собирается с
  `with_trusttunnel`, и в RouteBox появился соответствующий тип outbound:
  сервер, логин и пароль, TLS (SNI, самоподписанный сертификат, пропуск
  проверки, anti-DPI через фрагментацию TLS), QUIC с выбором алгоритма
  перегрузки и health check. Ссылки TrustTunnel импортируются как есть:
  deep link `tt://…` и connect-URL `https://…?d=…` разбираются на стороне
  RouteBox (новый `POST /api/outbounds/parse-link`), а подписки с такими
  ссылками подхватываются при обновлении. Endpoint с несколькими адресами
  раскладывается на несколько outbound-ов. Пункт в выборе протокола виден
  только когда бинарник собран с этим тегом.

### Changes

- **Конфиг переезжает на sing-box 1.15 сам.** В 1.15 всё, что в 1.14 было
  объявлено устаревшим, стало фатальной ошибкой на старте — и часть из этого
  `check` не видит, поэтому раньше preflight обновления проходил, а процесс
  падал и откатывался на 1.14. RouteBox при загрузке (и при ручном PUT
  конфига) переписывает: `download_detour` у наборов правил → `http_client`,
  общий клиент `http_clients["rule-sets"]` через `route.final` (так работал
  старый неявный клиент), явное прямое скачивание — отдельный клиент;
  `store_rdrc` → `store_dns`; встроенный `tls.acme` →
  `tls.certificate_provider`; убраны `route.default_domain_strategy` и
  `domain_strategy` у DNS-серверов. Формы пишут новые поля, валидатор
  отклоняет старые с подсказкой. Если установленный форк старше 1.14,
  миграция не выполняется.

  Порядок обновления: сначала RouteBox, потом amnezia-box
  (1.15.0-alpha.6-awgm.25 или новее). Старый RouteBox с новым форком
  упрётся в эти ошибки на старте.

- **Главная без лишнего (#108).** Заголовок «Dashboard» убран, отступы между
  блоками ужаты — на 24" в 1080p страница перестала прокручиваться. Блок
  разбивки рядом с графиком шире, кольцо крупнее и стоит по центру блока, а
  длинные имена цепочек помещаются целиком. При наведении на график линия и
  точки стали тише, а вместо «−7 min» показывается время точки.

### Fixes

- **Обновление amnezia-box из панели снова находит релизы.** Все релизы форка
  после переезда на 1.15 помечены как prerelease, а GitHub `releases/latest`
  такие пропускает и отвечает 404 — панель показывала «no releases found».
  Теперь берётся самый свежий опубликованный релиз из списка.

- **Apply замечает, что процесс умер на reload.** sing-box выходит по SIGHUP,
  если новую конфигурацию нельзя запустить (или сигнал пришёл до конца
  старта); RouteBox отвечал «hot reload», хотя процесса уже не было. Теперь
  после сигнала процесс наблюдается две секунды, и падение уходит в обычный
  перезапуск с внятной ошибкой.

- **Endpoint-ы: поля по типу.** У WireGuard-пира ключ `pre_shared_key`, у
  AmneziaWG — `preshared_key`; форма писала одно написание для обоих и не
  подхватывала `pre_shared_key` при редактировании. Опции `system`, `name`,
  `udp_timeout`, `workers` есть только у WireGuard — для AWG они больше не
  пишутся.

- **DNS-правила.** `ip_cidr` в DNS-правиле с 1.14 сравнивается с ответом и
  требует `match_response` — форма ставит его сама и подсказывает про правило
  `evaluate`; `disable_cache` больше не пишется у `reject`/`predefined`.

## [0.36.9]

### Changes

- **TUN: стек больше не задаётся — работает новый стек sing-box 1.15.** Форк
  amnezia-box переехал на 1.15.0-alpha.5 и собирается без `with_gvisor`: у
  sing-tun теперь собственный TCP/IP-стек, он заметно быстрее и легче по CPU и
  памяти, и включается ровно тогда, когда опция `stack` не указана. Сама опция
  объявлена устаревшей и исчезнет в 1.17.

  Поэтому выбор стека убран из формы TUN-входа, а RouteBox при старте
  вычищает `stack` из конфига на диске (и делает бэкап, как при любой своей
  записи) — иначе оставшийся `gvisor`/`mixed` просто не дал бы процессу
  подняться на сборке без gVisor. Конфиг с `stack: gvisor`/`mixed`, присланный
  в API вручную, теперь отклоняется с внятной ошибкой вместо падения при
  запуске.

  Обновите amnezia-box до 1.15.0-alpha.5-awgm.19 или новее.

## [0.36.8]

### Features

- **Версия модуля ядра AmneziaWG видна на странице AWG (#88).** Там, где раньше
  было только «модуль установлен / не установлен», теперь стоит и сама версия
  (`v1.0.20241018`), а когда модуля нет — заметка и ссылка на инструкцию по
  установке для прочих дистрибутивов. Версия и обе проверки возможностей ядра
  берутся из одного зондирования на опрос, а не из трёх. Пустой ответ модуля
  больше не выдаётся за «не установлен»: это отдельное состояние «установлен,
  версия недоступна» — прежде оно молча превращалось в предложение поставить
  уже стоящий модуль.

### Changes

- **Главная: разбивка рядом с графиком, CPU и память в подвале (#101).** Справа
  от графика теперь кольцо с переключателем «Клиенты / Цепочки». CPU и память
  переехали к общим значениям и диску; у CPU остался гриафк. Наведение на график показывает время и
  обе скорости в этой точке при этом плашка размещается внизу в строках легенды, на место
  подписи периода, а не бегает за курсором.

### Fixes

- **Несколько диапазонов портов в правиле (#104).** Поля «Диапазоны портов» и
  «Диапазоны портов источника» были однострочными, хотя в конфиг уходит список и
  разбор по строкам уже был: у Discord это `50000:50099` и `19200:19400`, и
  второй ввести было некуда. Теперь это такие же многострочные поля, как соседние
  условия, — по диапазону в строке (запятая тоже годится: в соседнем поле «Порт
  источника» список именно через запятую).

- **Правило больше не описывает себя как «весь трафик» (#104).**  Недостающие условия добавлены в обе сводки.

- **Тег набора правил, созданного прямо из формы правила, сохраняется.** 

- **Счётчик соединений рядом с адресом клиента (#107).** В свёрнутой строке
  «Соединений» число было прижато к правому краю ячейки и после короткого имени
  висело в пустоте. Теперь оно идёт сразу за адресом.

- **В «Разбивке» и «Клиентах» только локальные устройства этого бокса (#102).**  LAN, заданная подсеть
  AWG, 100.64.0.0/10, loopback и link-local остаются, остальное не показывается 
