...
# Все вопросы задаются до того, как что-то делается: оператор, ответивший и
# отошедший, не должен вернуться к скрипту, который ничего не сделал и ждёт.
# Значит установка модуля читает готовый ответ, а не спрашивает сама.
WANT_AWG_KERNEL="false"
OUT_NOASK="$( (maybe_install_awg_module) 2>&1 )"
assert_eq "" "$OUT_NOASK" "без согласия модуль не ставится и ничего не спрашивается"
